...
#     Based on Journal Impact Factor AND citation count
#     Quality_Score (1–3)
#
...
parse_score_response <- function(response_text) {
  ...
  if (last_semicolon > 0) {
    ...
    # No semicolon found — try to extract score from anywhere
...
extraction_df2<-extraction_df%>%
  select("File","NbS_Practice","NbS_Cluster","Extract_Status","Pages","NbS_Subpractice","Input_Variables","Analysis_Resolution","Spatial_Method","Method_Validation","Tool","Tool_Source","Output","Climate_Barrier","Climate_Benefit","Economics","Geographic_Scope","Narrative","Code_Data_Availability","Contacts_Email","Extractor","Scalability_A_Geographic","Scalability_A_Justification","Scalability_B_Method","Scalability_B_Justification","Scalability_C_Data","Scalability_C_Justification","Scalability_Score","doi_extracted","doi_lower")
...
#   1 = Journal IF < 2 AND fewer than 20 citations
#   2 = Journal IF >= 2 OR >= 20 citations (either condition met)
#   3 = Journal IF > 5 AND >= 20 citations (both conditions met)
#
# Papers with missing bibliometric data default to score 1
# ============================================================================

extraction_df <- extraction_df %>%
  mutate(
    # Handle NAs for clean logic
    IF_value  = replace_na(journal_impact_factor, 0),
    cit_value = replace_na(cited_by_count, 0),
    
    # --- Evidence Quality Score (exact methodology criteria) ---
    Quality_Score = case_when(
      # Score 3: IF > 5 AND >= 20 citations (both conditions)
      IF_value > 5 & cit_value >= 20                       ~ 3L,
      # Score 2: IF >= 2 OR >= 20 citations (either condition)
      IF_value >= 2 | cit_value >= 20                      ~ 2L,
      # Score 1: IF < 2 AND < 20 citations (neither condition)
      TRUE                                                 ~ 1L
    ),
...
cat("\n=== RAW BIBLIOMETRIC SUMMARY ===\n")
cat(glue("Impact Factor: median = {round(median(extraction_df$journal_impact_factor, na.rm=TRUE), 1)}, ",
         "mean = {round(mean(extraction_df$journal_impact_factor, na.rm=TRUE), 1)}, ",
         "range = {round(min(extraction_df$journal_impact_factor, na.rm=TRUE), 1)}",
         "–{round(max(extraction_df$journal_impact_factor, na.rm=TRUE), 1)}\n"))
cat(glue("Total citations: median = {round(median(extraction_df$cited_by_count, na.rm=TRUE), 0)}, ",
         "mean = {round(mean(extraction_df$cited_by_count, na.rm=TRUE), 0)}, ",
         "range = {round(min(extraction_df$cited_by_count, na.rm=TRUE), 0)}",
         "–{round(max(extraction_df$cited_by_count, na.rm=TRUE), 0)}\n"))
...
# ============================================================================
#
# From methodology document (exact matrix):
#
...
extraction_df %>%
  ...
  summarise(
    ...
    median_IF        = round(median(journal_impact_factor, na.rm = TRUE), 1),
    median_citations = round(median(cited_by_count, na.rm = TRUE), 0),
...
extraction_df %>%
  ...
  arrange(desc(Combined_Score), desc(cited_by_count)) %>%
  select(File, NbS_Practice, Scalability_Score, Quality_Score,
         Combined_Score, Final_Benchmark, journal_impact_factor,
         cited_by_count) %>%
  head(20) %>%
  print()

# --- Unmatched papers (no bibliometric data) ---
cat("\n=== PAPERS WITHOUT BIBLIOMETRIC DATA ===\n")
extraction_df %>%
...
message(glue("  - nbs_extraction_fully_benchmarked.rds"))
message(glue("  - nbs_extraction_fully_benchmarked.csv"))
message(glue("  - nbs_extraction_fully_benchmarked.xlsx"))
...
DT::datatable(
  extraction_df %>%
    select(File, NbS_Practice, NbS_Cluster,
           journal_name, journal_impact_factor, cited_by_count,
           Quality_Score,
           Scalability_A_Geographic, Scalability_A_Justification,
           Scalability_B_Method, Scalability_B_Justification,
           Scalability_C_Data, Scalability_C_Justification,
  ...
  caption = "NbS Benchmarking — Scalability + Evidence Quality"
)
```
...
if (exists("papers_to_score")) {
  ...
  total_calls <- n_papers * 3  # 3 scalability prompts per paper
  avg_chars <- mean(nchar(papers_to_score$Text_For_Scoring), na.rm = TRUE)
  avg_input_tokens <- avg_chars / 4
  avg_output_tokens <- 80  # slightly more due to justification-first format
