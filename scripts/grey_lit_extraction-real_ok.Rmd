...
# Model selection
GPT_MODEL <- "gpt-5"  # Cost-effective option
...
Extract ONLY the FINAL DECISION-FACING GEOSPATIAL PRODUCTS of the analysis.

DECISION RULE: For each map mentioned in the results, ask: "Does this map answer WHERE to implement NbS or HOW MUCH BENEFIT NbS provides?" If NO, exclude it.

INCLUDE:
- Suitability / priority / opportunity / allocation maps for NbS placement
- Spatially-explicit NbS benefit maps (carbon sequestration, yield gain, flood risk reduction, biodiversity gain, economic return)
- Prioritazation / Suitability / ranking Methodology produced

EXCLUDE even if shown in results:
- Any map of an environmental variable (slope, NDVI, soil, rainfall, temperature, runoff, drainage, tree cover, land cover, poverty, population, climate projections/anomalies, watershed boundaries, stream order, proximity layers)
- Constraint or exclusion masks
- Sub-scores or criterion layers

RULES:
- One suitability map with multiple classes = ONE item
- 1–5 items MAX, semicolon-separated, SINGLE LINE
- Short noun phrases with classes in brackets if stated
- If nothing qualifies, return NA
...
QUESTION
In their methods the authors explicitly treat climate hazards or climate change as a RISK or BARRIER that could limit the feasibility, performance, or long-term success of the proposed NbS in the output produced by the paper?

Say YES only if climate hazards are considered in the method as threatening suitability, persistence, or effectiveness of the intervention and that they treat it in their method or outputs. Narrative statements are not enough, it should be part of the method or result section of the paper. It should be a core section and not just a vague mention. 
...
QUESTION
In their method do the authors explicitly use climate hazards or climate change as a REASON to target areas where NbS provides adaptation or mitigation benefits?

Say YES only if NbS is spatially targeted because it can reduce climate risks or deliver mitigation outcomes and that they treat it in their outputs or methods. Narrative statements are not enough, it should be in the method or result section of the paper and should be a core section not just a vague mention. 
...
prompt_economics <- function(txt) {
  glue('
Read the following text from a peer-reviewed NbS spatial prioritisation paper.

TASK:
Determine whether the paper includes FINANCIAL or MONETARY economic evaluation in the analytical outputs (Methods or Results).

STRICT DEFINITION:
"Economic" here refers ONLY to financial/monetary metrics or cost-based optimisation.

COUNT AS "YES" ONLY IF at least one of the following is explicitly implemented in Methods or Results:

1) Monetary valuation or cost metrics:
   - Implementation cost ($)
   ...
   - Explicit cost surfaces or net benefit maps expressed in monetary terms

2) An optimisation or decision rule that explicitly minimises cost or maximises net financial benefit.

DO NOT COUNT AS "YES" under any circumstance:
- Poverty maps
- Livelihood targeting
- Socioeconomic vulnerability layers
- Proximity analysis with poverty
- Social benefit framing without monetary valuation
- Suitability layers containing socioeconomic variables
- Any non-monetary social indicator

If no explicit monetary or cost-based metric is implemented analytically in Methods or Results, output:
...
Single line only.
If YES:
"Yes: explain the financial method and metrics"
If NO:
"No"
...
# Save as CSV
write_excel_csv(extraction_results, file.path(OUT_DIR, "nbs_extraction_results_grey_lit_test.csv"))
